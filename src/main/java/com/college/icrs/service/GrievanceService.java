...
public class GrievanceService {
    ...
    private final GrievanceCommentService grievanceCommentService;
    ...
    public Grievance updateGrievance(Long id, Grievance grievanceDetails) {
        ...
        applyGrievanceDetails(grievance, grievanceDetails);
    ...
    public Grievance applyAiDecisionMetadata(
        ...
        Grievance grievance = getGrievanceById(grievanceId);
        applyAiMetadata(grievance, priority, sentiment, aiTitle, aiConfidence, aiModelName, aiDecisionSource, aiDecisionAt);
        Grievance saved = grievanceRepository.save(grievance);
    ...
    public CommentResponseDTO addSystemComment(
        ...
        Grievance grievance = getGrievanceById(grievanceId);
        return grievanceCommentService.addSystemComment(grievance, systemAuthorEmail, body);
    }

    public CommentResponseDTO addComment(Long grievanceId, String authorEmail, String body) {
        Grievance grievance = getGrievanceById(grievanceId);
        return grievanceCommentService.addComment(grievance, authorEmail, body);
    }

    @Transactional(readOnly = true)
    public List<CommentResponseDTO> getComments(Long grievanceId, String requesterEmail) {
        Grievance grievance = getGrievanceById(grievanceId);
        return grievanceCommentService.getComments(grievance, requesterEmail);
    }
    ...
    private void prepareNewGrievance(Grievance grievance) {
        grievance.setStatus(Status.SUBMITTED);
        initializeAiFieldsForNewGrievance(grievance);
        applyDefaultAssignee(grievance);
    }

    private void applyDefaultAssignee(Grievance grievance) {
        ...
        if (grievance.getAssignedTo() == null && grievance.getCategory() != null && grievance.getCategory().getDefaultAssignee() != null) {
            grievance.setAssignedTo(grievance.getCategory().getDefaultAssignee());
            grievance.setStatus(Status.IN_PROGRESS);
        }
    }

    private void applyGrievanceDetails(Grievance grievance, Grievance grievanceDetails) {
    ...
    private void applyResolvedByAi(
            Grievance grievance,
            String aiResolutionText,
            String aiResolutionComment,
            Double aiConfidence,
